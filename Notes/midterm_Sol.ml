...
7 8 9

. We say that a matrix is proper if every row
has the same length and square if it is proper and the length of each row is equal to the
number of rows. The example above is a (proper) square matrix. Write a program to test
whether a given list of lists is a square matrix.
Solution to Q2:
let square m =
match m with
| [] -> true
